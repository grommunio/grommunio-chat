...
describe('Customization', () => {
    ...
    it('MM-T1215 - Can change grommunio-chat Apps Download Page Link setting', () => {
        // * Verify grommunio-chat Apps Download Page Link's setting name is visible and matches the text
        cy.findByTestId('NativeAppSettings.AppDownloadLinklabel').scrollIntoView().should('be.visible').and('have.text', 'grommunio-chat Apps Download Page Link:');

        // * Verify the grommunio-chat Apps Download Page Link input box has default value. The default value depends on the setup before running the test.
        ...
        cy.findByTestId('NativeAppSettings.AppDownloadLinkhelp-text').find('span').should('be.visible').and('have.text', 'Add a link to a download page for the grommunio-chat apps. When a link is present, an option to "Download grommunio-chat Apps" will be added in the Product Menu so users can find the download page. Leave this field blank to hide the option from the Product Menu.');
    ...
    it('MM-T1209 - Can change Help Link setting', () => {
        // * Verify that setting is visible and matches text content
        const contents = ['The URL for the Help link on the grommunio-chat login page, sign-up pages, and Help Menu. If this field is empty, the Help link is hidden from users.'];
    ...
    it('MM-T1213 Can change About Link setting', () => {
        ...
        // * Verify that the help text is visible and matches text content
        cy.findByTestId('SupportSettings.AboutLinkhelp-text').should('be.visible').and('have.text', 'The URL for the About link on the grommunio-chat login and sign-up pages. If this field is empty, the About link is hidden from users.');
    ...
    it('MM-T1211 - Can change Terms of Use Link setting', () => {
        ...
        cy.findByTestId('SupportSettings.TermsOfServiceLinkhelp-text').find('span').should('be.visible').and('have.text',
            ...
            '"grommunio-chat Acceptable Use Policy" explaining the terms under which grommunio-chat software is ' +
            'provided to end users. If you change the default link to add your own terms for using the service you ' +
            'provide, your new terms must include a link to the default terms so end users are aware of the grommunio-chat ' +
            'Acceptable Use Policy for grommunio-chat software.');
