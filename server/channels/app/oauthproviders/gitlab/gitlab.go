...
func gitLabUserFromJSON(logger mlog.LoggerIFace, data io.Reader) (*GitLabUser, error) {
	...
	logger.Debug(
		"Converted JSON data to GitLab user",
		mlog.Int("Id", glu.Id),
...
func (gp *GitLabProvider) IsSameUser(c request.CTX, dbUser, oauthUser *model.User) bool {
	if dbUser.AuthData == nil || oauthUser.AuthData == nil {
		c.Logger().Debug(
			"gitlab.IsSameUser: AuthData is nil",
			mlog.String("dbUser.Email", dbUser.Email),
			mlog.String("oauthUser.Email", oauthUser.Email),
			mlog.Bool("dbUser.AuthData == nil", dbUser.AuthData == nil),
			mlog.Bool("oautUser.AuthData == nil", oauthUser.AuthData == nil),
		)
		return false
	}
	c.Logger().Debug(
		"gitlab.IsSameUser: Comparing user data",
		mlog.String("dbUser.Email", dbUser.Email),
		mlog.String("oauthUser.Email", oauthUser.Email),
