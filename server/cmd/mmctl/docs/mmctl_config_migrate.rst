...
Migrate a file-based configuration to (or from) a database-based configuration. Point the grommunio-chat server at the target configuration to start using it. Note that this command is only available in `--local` mode.
